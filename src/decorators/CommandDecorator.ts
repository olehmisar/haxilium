...
export function CommandDecorator(name: string, access?: string, meta?: any): CommandMethodDecorator
export function CommandDecorator(names: string[], access?: string, meta?: any): CommandMethodDecorator
export function CommandDecorator(nameOrNames: string | string[], access?: string, meta?: any): CommandMethodDecorator {
    const names = Array.isArray(nameOrNames) ? nameOrNames : [nameOrNames]

    return (target, key) => {
        const commands: [string | symbol, CommandOptions][] | undefined = Reflect.getMetadata('haxball-room:commands', target)
        const command: ArrayValuesType<NonNullable<typeof commands>> = [key, { names, access, meta }]
        if (commands) {
